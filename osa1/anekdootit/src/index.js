import React, { useState } from 'react';
import ReactDOM from 'react-dom';
import './index.css';

const Anecdote = ({ text, votes }) => (
  <>
    <h1>Anecdote of the day</h1>
    <p>{text}</p>
    <p>this anecdote has {votes} votes</p>
  </>
)

const Button = ({ onClick, text }) => (
  <button onClick={onClick}>
    {text}
  </button>
)

const Favorite = ({ text, votes }) => (
  <>
    <h1>Anecdote with most votes</h1>
    <p>{text}</p>
    <p>this anecdote has {votes} votes</p>
  </>
)

const App = (props) => {
  const [selected, setSelected] = useState(0)
  const [points, setPoints] = useState(new Array(anecdotes.length).fill(0))
  const top = points.reduce((iMax, x, i, arr) => x > arr[iMax] ? i : iMax, 0);
  const favorite = {
    text: props.anecdotes[top],
    votes: points[top]
  }
  const vote = () => {
    const copy = [...points]
    copy[selected] += 1
    setPoints(copy)
    //setPoints({ ...points, points[selected] += 1 })
  }
  const next = () => {
    const rand = Math.floor(Math.random() * props.anecdotes.length)
    return(rand)
  }
  return (
    <div>
      <Anecdote text={props.anecdotes[selected]} votes={points[selected]} />
      <Button onClick={vote} text='vote' />
      <Button onClick={() => setSelected(next)} text='next anecdote' />
      <Favorite text={favorite.text} votes={favorite.votes} />
    </div>
  )
}

const anecdotes = [
  'If it hurts, do it more often',
  'Adding manpower to a late software project makes it later!',
  'The first 90 percent of the code accounts for the first 90 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.',
  'Any fool can write code that a computer can understand. Good programmers write code that humans can understand.',
  'Premature optimization is the root of all evil.',
  'Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.'
]


ReactDOM.render(
  <App anecdotes={anecdotes} />,
  document.getElementById('root')
)